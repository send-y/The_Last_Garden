# Документация проекта

Эта папка — основной источник актуальных проектных решений. Обсуждения в чатах полезны для поиска идей, но решение считается зафиксированным только после внесения сюда. Текущий статус реализации и ручной проверки хранится в [дорожной карте](roadmap.md).

## Статусы решений

- **Принято** — направление согласовано; менять только осознанным новым решением.
- **Гипотеза** — правдоподобное решение, которое нужно проверить прототипом или плейтестом.
- **Открыто** — важная развилка без выбранного варианта.
- **Отложено** — не входит в ближайший объём работ, но сохраняется для будущего.
- **Отменено** — решение больше не действует; причина остаётся в журнале.

## Документы

### Основа

- [Видение игры](vision.md)
- [Handoff: состояние проекта для независимого анализа (25.09.2026)](handoff-2026-09-25.md)
- [Черновик основного игрового цикла](core-game-loop.md)
- [Журнал дизайнерских решений](design-decisions.md)
- [Открытые вопросы](open-questions.md)
- [Архитектурные принципы](architecture-principles.md)
- [Рынок, позиционирование и границы продукта](market-and-positioning.md)
- [Дорожная карта](roadmap.md)
- [Технический план первого среза](implementation/first-slice-technical-plan.md)
- [Технический план 2A: первый NPC](implementation/first-npc-technical-plan.md)
- [Технический план Mechanics Lab](implementation/mechanics-lab-technical-plan.md)
- [Технический план L2: фундамент локализации](implementation/localization-foundation-technical-plan.md)
- [Технический план 2B: потребности, движение и распорядок](implementation/npc-needs-schedule-technical-plan.md)
- [Технический план C1: фундамент строительства](implementation/construction-foundation-technical-plan.md)
- [Технический план 2C: самостоятельная работа и просьба о помощи](implementation/npc-construction-assistance-technical-plan.md)
- [Технический план 2D: память, отношения и контекстная реплика](implementation/npc-memory-relations-technical-plan.md)
- [Технический план C2: игровой цикл строительства](implementation/gameplay-construction-technical-plan.md)

### Системы

- [Игрок и поселение](systems/player-and-settlement.md)
- [Мир и путешествия](systems/world-and-travel.md)
- [Время, климат и сезоны](systems/time-and-seasons.md)
- [Работа, ремесло и исследования](systems/work-crafting-and-research.md)
- [NPC, отношения и диалоги](systems/npc-relationships-and-dialogue.md)
- [Модель отношений NPC](systems/npc-relationship-model.md)
- [Традиции и праздники](systems/traditions-and-holidays.md)
- [Угрозы, нападения и восстановление](systems/threats-and-recovery.md)
- [Финал и судьба игрока](systems/endgame-and-player-fate.md)
- [Сеттинг и технологии](systems/setting-and-technology.md)
- [Ресурсы, фермерство и рыбалка](systems/resources-farming-and-fishing.md)
- [Строительство и области](systems/construction-and-areas.md)
- [Потребности, питание и настроение](systems/needs-food-and-mood.md)
- [Работа NPC и обязательства](systems/npc-work-and-commitments.md)
- [Общий вклад и справедливость](systems/settlement-economy-and-fairness.md)
- [Визуальное направление](systems/art-direction.md)
- [Начало игры и первый вертикальный срез](systems/opening-and-first-slice.md)

## Правило обновления

После каждого содержательного дизайнерского обсуждения нужно:

1. Обновить соответствующий системный документ.
2. Добавить или изменить запись в журнале решений.
3. Перенести нерешённые последствия в список открытых вопросов.
4. Если решение требует практической проверки, указать её в дорожной карте.

## Новое направление хозяйственного цикла (21 сентября 2026)

- [Бездна, растения, баллоны и экспедиции](systems/abyss-and-expeditions.md) — согласованные правила, ориентиры баланса и открытые вопросы.

- [Рабочий каталог предметов](systems/item-catalog.md) — существующие игровые данные, предметы из дизайна и кандидаты для обсуждения.

- [Принятые цепочки крафта и хозяйства](systems/crafting-chains.md) — станции, материалы, еда, инструменты, грядки и точка продолжения следующего обсуждения.
